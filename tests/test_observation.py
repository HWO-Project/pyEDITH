...
def test_load_configuration(caplog):
    with caplog.at_level(logging.DEBUG, logger="pyEDITH"):
        ...
        assert np.all(
            obs.wavelength[obs.wavelength.value < parameters["lam_high"][0]]
            / obs.delta_wavelength[obs.wavelength.value < parameters["lam_high"][0]]
            == parameters["spectral_resolution"][0]
        )
        assert np.all(
            obs.wavelength[obs.wavelength.value >= parameters["lam_high"][1]]
            / obs.delta_wavelength[obs.wavelength.value >= parameters["lam_high"][1]]
            == parameters["spectral_resolution"][1]
        )
